...
async fn main(spawner: Spawner) {
    ...
    join(runner.run(), async {
        ...
        loop {
            ...
            let res = stack.send(to_address, Flags::Ack(3), &[0xAA, counter as u8]).await;
            log::info!("Tx Res {:?}", res);

            counter += 1;
            log::info!("Trying to receive packet for 10 seconds (# {})", counter);
            let rx_result = with_timeout(Duration::from_secs(10), stack.recv()).await;
            match rx_result {
                Ok(packet) => log::info!("Rx Packet {:?}", packet),
