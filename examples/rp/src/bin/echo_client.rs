...
async fn main(spawner: Spawner) {
    ...
    // Run the radio Runner concurrently with the application loop. Both
    // futures are infinite; join blocks main forever, which is what we want.
    join(runner.run(), async {
        ...
        loop {
            ...
            let res = stack.send(to_address, Flags::Ack(3), &data_to_send).await;
            log::info!("Tx Res {:?}", res);

            // now expect an echo with same counter, but probably other flags
            log::debug!("Expecting echo within 1 second (# {})", counter);
            let rx_result = with_timeout(Duration::from_secs(1), stack.recv()).await;
            match rx_result {
                Ok(packet) => {
