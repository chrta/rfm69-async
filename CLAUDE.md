...
Toolchain layout:
- The driver crate (`rfm69-async/`) declares its MSRV via `rust-version = "1.88"` in `Cargo.toml`; the rationale for that floor lives once, in the crate-root rustdoc `# MSRV` section (`src/lib.rs`). CI's `msrv` job runs `cargo msrv verify` (default + `--all-features`), which reads the floor from the manifest so the two can't drift. The driver has no `rust-toolchain.toml` and no target requirement — it builds on whatever stable users have, host or cross.
...
cargo +1.88 build --all-features  # MSRV gate (floor: see src/lib.rs `# MSRV`)
